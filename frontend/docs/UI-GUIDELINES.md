...
calendar: `new` → blue, `in_progress` → **amber**, `done` → **emerald**, `cancelled` → muted, `deferred` → muted.
...
## 17. Shared presentation primitives

Canonical rules for the reusable presentation primitives in `src/components/ui`. Reach for
these instead of hand-rolling the same markup so empties, statuses, severities and loading
states stay consistent across every product surface.
