...
function seg(overrides: Partial<ResourceUtilizationSegment> = {}): ResourceUtilizationSegment {
  return {
...
describe('ResourceSegmentBar', () => {
  ...
  it('fires onClick with the segment when clicked', () => {
    ...
    fireEvent.click(screen.getByTestId('resource-segment-bar'));
  ...
  it.each(['Enter', ' '])('fires onClick on %s key', (key) => {
    const s = seg();
    const onClick = renderBar(s);
    fireEvent.keyDown(screen.getByTestId('resource-segment-bar'), { key });
    expect(onClick).toHaveBeenCalledWith(s);
  });
  ...
  it('hides the inline label for a narrow segment but keeps the tooltip', () => {
    ...
    expect(screen.getByTestId('resource-segment-bar')).toHaveAttribute('title');
  });
  ...
  it('pluralises the assignment count in the tooltip', () => {
    ...
    expect(screen.getByTestId('resource-segment-bar').getAttribute('title')).toContain(
      '1 assignment',
    );
  });
  ...
  it('includes "capability conflict" in the tooltip when hasConflict is true', () => {
    renderBar(seg(), vi.fn(), 0, true);
    const title = screen.getByTestId('resource-segment-bar').getAttribute('title');
