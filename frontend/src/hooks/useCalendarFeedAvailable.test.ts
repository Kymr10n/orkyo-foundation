...
/**
 * Calendar subscriptions delegates to useFeatureEnabled — the exhaustive cases (site admin,
...
describe('useCalendarFeedAvailable', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('returns true when the server entitles CalendarFeed', () => {
    mockUseAuth.mockReturnValue(authState({ [FeatureKeys.CalendarFeed]: true }));
    expect(renderHook(() => useCalendarFeedAvailable()).result.current).toBe(true);
  });

  it('returns false when the server does not entitle CalendarFeed', () => {
    mockUseAuth.mockReturnValue(authState({ [FeatureKeys.CalendarFeed]: false }));
    expect(renderHook(() => useCalendarFeedAvailable()).result.current).toBe(false);
  });
  ...
  it('is not influenced by the plan code, in either spelling', () => {
    ...
    mockUseAuth.mockReturnValue(
      authState({}, { tier: 'Enterprise', entitlements: { [FeatureKeys.CalendarFeed]: true } }),
    );
    expect(renderHook(() => useCalendarFeedAvailable()).result.current).toBe(true);

    mockUseAuth.mockReturnValue(
      authState({}, { tier: 'enterprise', entitlements: { [FeatureKeys.CalendarFeed]: false } }),
    );
    expect(renderHook(() => useCalendarFeedAvailable()).result.current).toBe(false);
  });
});
