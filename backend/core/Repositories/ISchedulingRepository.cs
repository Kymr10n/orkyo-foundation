...
public interface ISchedulingRepository
{
    ...
    /// <summary>Returns the anchoring site for the given resource (resources.home_site_id),
    /// or <c>null</c> if the resource has no site.</summary>
    Task<Guid?> GetSiteIdForResourceAsync(Guid resourceId, CancellationToken ct = default);

    /// <summary>Bulk resource→anchoring-site map (home_site_id) in one query — for batch validation.</summary>
