...
{
    Task<List<ResourceTypeInfo>> GetAllAsync();
    Task<ResourceTypeInfo?> GetByIdAsync(Guid id);
    Task<ResourceTypeInfo?> GetByKeyAsync(string key);
}
    ...
    : IResourceTypeRepository
{
    ...
    public async Task<List<ResourceTypeInfo>> GetAllAsync()
    {
    ...
    public async Task<ResourceTypeInfo?> GetByIdAsync(Guid id)
    {
        await using var db = connectionFactory.CreateOrgConnection(orgContext);
        await db.OpenAsync();
    ...
    public async Task<ResourceTypeInfo?> GetByKeyAsync(string key)
    {
        await using var db = connectionFactory.CreateOrgConnection(orgContext);
        await db.OpenAsync();
