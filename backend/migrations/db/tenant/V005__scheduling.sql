-- V005: Scheduling settings and off-times
-- Adds calendar-aware scheduling support per site.

-- ── Scheduling Settings (one per site) ──────────────────────────────
CREATE TABLE public.scheduling_settings (
    ...
    CONSTRAINT scheduling_settings_site_id_fkey FOREIGN KEY (site_id) REFERENCES public.sites(id) ON DELETE CASCADE,
    CONSTRAINT scheduling_settings_working_hours_order CHECK (working_day_start < working_day_end)
);
...
-- ── Off-Time ↔ Space junction (when applies_to_all_spaces = false) ──
CREATE TABLE public.off_time_spaces (
    ...
    CONSTRAINT off_time_spaces_off_time_fkey FOREIGN KEY (off_time_id) REFERENCES public.off_times(id) ON DELETE CASCADE,
    CONSTRAINT off_time_spaces_space_fkey FOREIGN KEY (space_id) REFERENCES public.spaces(id) ON DELETE CASCADE
);

-- ── Add scheduling_settings_apply flag to requests ──────────────────
ALTER TABLE public.requests
    ADD COLUMN scheduling_settings_apply boolean NOT NULL DEFAULT true;
